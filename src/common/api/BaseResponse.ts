...
export const Response = async (requestConfig: RequestConfigFields): Promise<AxiosResponse<any, any>> => {
  try {
    ...
    Allure.logStep(`Response Status = ${error.status}`);
    Allure.attachment(`Error Response Data for ${error.config.url}`, JSON.stringify(error.data, null, 2));
    return error;
  }
};
...
axios.interceptors.response.use(
  async (res) => {
    Allure.logStep(`Response Status = ${res.status}`);
